...
import random
# from texture_synthesis import generateTextureMap
import os
# import numpy as np
# import cv2
...
def generate_seamless_rain_texture(rain_patches, blocksize=100, overlap=10, outH=300, outW=300, tolerance=0.1):
    base_texture = random.choice(rain_patches)
    synthesized_texture = generateTextureMap(base_texture, blocksize, overlap, outH, outW, tolerance)
    return synthesized_texture.astype(np.uint8)

def blend_rain_texture(target_image, rain_texture, intensity_scale=1.0):
    resized_rain = cv2.resize(rain_texture, (target_image.shape[1], target_image.shape[0]))
    residual = resized_rain - np.mean(resized_rain, axis=(0, 1))
    blended = np.clip(target_image + residual * intensity_scale, 0, 255).astype(np.uint8)
    return blended

def generate_rainy_image(image, rain_patches, rain_type="light", blocksize=100):
    if not isinstance(image, np.ndarray):
        image = np.array(image)
    
    outH, outW = image.shape[:2]
    rain_texture = generate_seamless_rain_texture(rain_patches, blocksize, outH=outH, outW=outW)
    rain_gray = cv2.cvtColor(rain_texture, cv2.COLOR_BGR2GRAY)
    rain_colored = cv2.cvtColor(rain_gray, cv2.COLOR_GRAY2BGR)

    intensity_scale = 1.0 if rain_type == "light" else 0.7
    return blend_rain_texture(image, rain_colored, intensity_scale=intensity_scale)
